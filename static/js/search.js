...
export function initialize() {
    ...
    $("#search_query").on("blur", (e) => {
        // This blur event is a bit of a pain. The intent is that if the user
        // clicks out of the searchbox in a non-searching narrow, the searchbox
        // will close and show the narrow description. The part that is annoying
        // is that this can break the typeahead by interfering with the
        // selection via mouse click, ie selecting an item from the typeahead
        // will be buggy. To prevent this bug, we check that the **element that
        // received focus'** parent isn't the typeahead.
        if (e.relatedTarget === null || e.relatedTarget === undefined || $(e.relatedTarget).parents(".search_typeahead").length !== 1) {
