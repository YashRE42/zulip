    ...
    mock_template("presence_additional_count.hbs", false, (data) => {
        switch (additional_count_template_calls) {
            ...
            case 1:
                ...
                break;
            default:
                throw new Error("unexpected additional count template render call");
        }
...
run_test("find_row w/force_render", ({override}) => {
    ...
    const empty_user_row = buddy_list.find_row({
        key: user_key,
    });
    assert.equal(empty_user_row, stub_row);
    assert.ok(!user_shown);

    const user_row = buddy_list.find_row({
        key: user_key,
        force_render: true,
    });

    assert.equal(user_row, stub_row);
    assert.ok(user_shown);
    ...
    const empty_other_row = buddy_list.find_row({
        key: other_key,
    });
    assert.equal(empty_other_row, stub_row);
    assert.ok(!other_shown);

    const other_row = buddy_list.find_row({
        key: other_key,
        force_render: true,
    });

    assert.equal(other_row, stub_row);
    assert.ok(other_shown);
});
