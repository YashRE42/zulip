...
const emoji = mock_esm("../../static/shared/js/emoji");
...
const empty_$elem = $("#empty_elem");
empty_$elem[0] = empty_elem;
...
test("handlers", ({override, override_rewire, mock_template}) => {
    ...
    mock_template("user_presence_rows.hbs", false, () => to_$_fake_html);
...
test("mouseenter/mouseleave handlers", ({override}) => {
    ...
    fred_stub[0] = {
        to_$: () => fred_stub,
        addEventListener: (type, handler) => fred_stub.on(type, handler),
    };
